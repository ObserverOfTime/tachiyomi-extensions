...
abstract class MangAdventure(
    ...
    protected open val statuses =
        arrayOf("Any", "Completed", "Ongoing", "Hiatus", "Canceled")

    /** The site's sort order labels that correspond to [SortOrder.values]. */
    protected open val orders =
        arrayOf("Title", "Views", "Latest upload", "Chapter count")
    ...
    override fun getMangaUrl(manga: SManga) =
        "$baseUrl/reader/${manga.url}"

    override fun getChapterUrl(chapter: SChapter) =
        "$apiUrl/chapters/${chapter.url}/read"
